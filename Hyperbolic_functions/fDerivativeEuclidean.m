function Y = fDerivativeEuclidean(X,varargin)
...
dist = sqrt(X(range,1).^2 + (X(:,1)').^2 - 2*(X(range,1)*X(:,1)').*cos_angle);

Y(range,:,1) = 1./dist.*(X(range,1)-X(:,1)'.*cos_angle);

for count_angle = p:-1:2
    if count_angle == p
        Y(range,:,count_angle) = 1./dist.*(X(range,1)*X(:,1)').*sin(X(range,count_angle)-X(:,count_angle)') ;
        cos_angle = cos(X(range,count_angle)-X(:,count_angle)');
    else
        Y(range,:,count_angle) = -1./dist.*(X(range,1)*X(:,1)').* ...,
