% Hyperbolic metric, old loss, update radius, when R>Rmax, set R = Rmax
% Can Add RefPos

function [Y,stress,disparities,iter] = fmdscale_hyperbolic(D,p,rmax,MinRatio,varargin)
...
paramNames = {'RefPos','criterion' 'weights' 'start' 'replicates' 'UpdateRadius' 'options'};
paramDflts = {[] [] [] [] [] [] []};
[RefPos, criterion,weights,start,nreps,UpdateRadius,options] = ...
                           internal.stats.parseArgs(paramNames, paramDflts, varargin{:});
D = double(D);
[n,m] = size(D);

if isempty(UpdateRadius)
    UpdateRadius = true;
end
...
for rep = 1:nreps
   ...
   RefN = size(RefPos,1);
   if N <= RefN
       error('Two many reference points.')
   end
   % Sample new points
    NewPos = fSamplingHyperbolicUnif(N-RefN,p,rmax,MinRatio);
    Y0 = [RefPos;NewPos];
%     Y0 = fSamplingHyperbolic(N,p,rmax,MinRatio);
    ...
    if metric
        if strain
            [Y,stress,iter] = MDS(Y0,rmax,RefN,A,weights,strainFun,metric,weighted,UpdateRadius,options);
        else
            [Y,stress,iter] = MDS(Y0,rmax,RefN,dissimilarities,weights,stressFun,metric,weighted,UpdateRadius,options);
        end
        if nargout > 2
            disparities = dissimilarities;
        end
    else
   [Y,stress,iter,disparities] = MDS(Y0,rmax,RefN,dissimilarities,weights,stressFun,metric,weighted,UpdateRadius,options);
    end
    
    % Save this solution if it's the best one so far.
    if stress(end) < bestStress(end)
        bestStress = stress;
        bestY = Y;
...
function [Y,Allstress,iter,disparities] = ...
             MDS(Y,rmax,RefN,dissimilarities,weights,stressFun,metric,weighted,UpdateRadius,options)
...
%     Y = Y - repmat(mean(Y,1),n,1);
    ...
    if ~metric
        ...
        distances = squareform(fDistNative(Y));
...
%         scale = (max(dissimilarities) - max(distances))/2;       
%         Y(:,1) = Y(:,1) - scale;
%         Y(Y(:,1)<0,:) = 0;
...
%         
    ...
    [stress,grad] = feval(stressFun, Y, RefN, disparities, weights, UpdateRadius);
    normGrad = norm(grad(:));
...
%         disp(iter)
    ...
    maxStepLen = 2;
    while true
        stress = lineSearchCrit(maxStepLen,Y,RefN,stepDir,disparities,weights,UpdateRadius,stressFun);
        if stress > oldStress || isnan(stress), break; end
        maxStepLen = 2*maxStepLen;
    end
    [alpha, stress, err, output] = ...
            fminbnd(@lineSearchCrit,0,maxStepLen,lineSearchOpts, ...
                    Y,RefN,stepDir,disparities,weights,UpdateRadius,stressFun);
    if (err == 0)
    ...
    elseif (stress > oldStress)
        ...
        while true
            ...
            stress = lineSearchCrit(alpha,Y,RefN,stepDir,disparities,weights,UpdateRadius,stressFun);
    ...
    temp = Y(:,1);
    Y(RefN+1:end,:) = Y(RefN+1:end,:) + Ystep(RefN+1:end,:);
%     Y = Y + Ystep;
        Y(Y(:,1)>rmax,1) = rmax;
    Y(Y(:,1)<=0,1) = temp(Y(:,1)<=0,1);
    iter = iter + 1;
    temp_stress(iter) = stress;
...
Allstress = temp_stress;
%==========================================================================

function val = lineSearchCrit(t,Y,RefN,stepDir,disparities,weights,UpdateRadius,stressFun)
...
val = feval(stressFun,Y+t*stepDir,RefN,disparities,weights, UpdateRadius);


%==========================================================================

function [S,grad] = stressCrit(Y,RefN,disparities,weights,UpdateRadius)
...
if nargout > 1
    ...
    if sumDiffSq > 0
        if all(distances > 0)
               dS = squareform(weights.* ...
                     (diffs./sumDiffSq - distances./sumDistSq) );
%                 dS = squareform(diffs./sqrt(sumDiffSq));
                 dY = fDerivativeNative(Y);
            for i = 1:p
                
                grad(:,i) = sum(dS.*dY(:,:,i),2) .* S/2; 
%                 grad(:,i) = sum(dS.*dY(:,:,i),2); 
                if ~UpdateRadius
                    grad(:,1) = 0;
                end                  
            end       
...
function [S,grad] = metricStressCrit(Y,RefN,dissimilarities,weights,UpdataRadius)
...
if nargout > 1
    ...
    if sumDiffSq > 0
        if all(distances > 0)
            ...
            for i = 1:p
%                  grad(:,i) = sum(dS.*dY(:,:,i),2) .* S/2;    
                grad(:,i) = sum(dS.*dY(:,:,i),2) ./ (sumDissSq.*S);
            end
        else
            error(message('stats:mdscale:ColocatedPoints'));
        end
...
function [S,grad] = sstressCrit(Y,RefN,disparities,weights,UpdataRadius)
...
function [S,grad] = metricSStressCrit(Y,RefN,dissimilarities,weights,UpdataRadius)
...
function [S,grad] = sammonCrit(Y,RefN,dissimilarities,weights,UpdataRadius)
