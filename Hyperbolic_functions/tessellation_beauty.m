...
plot(x_h,y_h,'-','Linewidth',2/sqrt(1))
rotate_n_plot([x_h' y_h'],q,2/sqrt(1));
plot([0,s],[0,0],'Linewidth',2/sqrt(1))
rotate_n_plot([0 0;s 0],q,2/sqrt(1));

plot(0,0,'k.','Markersize',15)
plot(s,0,'.','Markersize',15/sqrt(1))
...
%                 end
    ...
    if size(xyr,1)==3
        ...
        plot(x_h,y_h,'-','Linewidth',2/sqrt(layer))
...
for ii = 1:size(current_node,1)-1
    ...
    plot(current_node(ii,1),current_node(ii,2),'.','Markersize',15/sqrt(layer))
...
plot(current_node(end,1),current_node(end,2),'.','Markersize',15/sqrt(layer))
